...
@section('content')
<br>
<h2 class="col-md-12 text-center"> Vehicle Type</h2>
<br>
<div class="container">
  <form action="/addvehicle" method="post">
    @csrf
    <div class="form-group row">
      <label for="inputmetrics" class="col-sm-2 col-form-label">Vehicle Name</label>
      <div class="col-sm-10">
        <input type="text" class="form-control" name="name" id="vehicle_id" placeholder="van">
      </div>
    </div>
    ...
    <tbody>
      ...
      <tr>
        ...
        @if($val->isActive == true)
          <td><a href="/deletevehicle/{{$val->id}}" class="btn btn-outline-danger">Delete</a></td>
        @else
           <td><a href="/activevehicle/{{$val->id}}" class="btn btn-outline-primary">Active</a></td>
        @endif
      </tr>
      @endforeach
    </tbody>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header text-center">
        <h4 class="modal-title w-100 font-weight-bold">Vehicle Update</h4>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
        <form id="" method="post" action="/updatevehicle" enctype="multipart/form-data">
          {{ csrf_field() }}
